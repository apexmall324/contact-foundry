...
async function shareVcf(file) {
    const blob = new Blob([buildVcf(file.contacts)], { type: "text/vcard;charset=utf-8" });
    const shareFile = new File([blob], file.filename, { type: "text/vcard" });
    if (navigator.canShare && navigator.canShare({ files: [shareFile] })) {
        try {
            await navigator.share({ files: [shareFile], title: file.filename });
        } catch (error) {
            if (error.name !== "AbortError") window.alert("Sharing was cancelled or failed.");
        }
        return;
    }
    window.alert("Sharing isn't supported in this browser. Downloading the file instead.");
    downloadVcf(file);
}
...
function renderArchive() {
    ...
    files.forEach((file) => {
        ...
        row.innerHTML = `<td class="date">${formatDate(file.createdAt)}</td><td class="filename"></td><td><span class="badge">${file.contacts.length}</span></td><td><div class="actions"><a class="text-link download-link" href="#">Download</a><a class="text-link share-link" href="#">Share</a><a class="text-link" href="manage.html?id=${encodeURIComponent(file.id)}">Edit list</a></div></td>`;
        row.querySelector(".filename").textContent = file.filename;
        row.querySelector(".download-link").addEventListener("click", (event) => {
            event.preventDefault();
            downloadVcf(file);
        });
        row.querySelector(".share-link").addEventListener("click", (event) => {
            event.preventDefault();
            shareVcf(file);
        });
